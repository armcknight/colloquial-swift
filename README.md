...
- cli commands for extensions
	- count all extension declarations grouped by uniq: `jq '.declarations.extension.parsed[].identifier' observations/*.json | sort | uniq -c | sort`
		- count all extension declarations not on Cocoa frameworks (so, no AVFoundation/UIKit/etc): `jq '.declarations.extension.parsed[].identifier' observations/*.json | sed s/\"//g | grep -v -e "^UI.*" -e "^NS.*" -e "^CG.*" -e "^CI.*" -e "^CL.*" -e "^MK.*" -e "^AV.*" | sort | uniq -c | sort`
